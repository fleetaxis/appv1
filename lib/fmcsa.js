...
function parseSaferSnapshot(html, dotHint) {
  ...
  if (!fullText || /no records? matching|record not found/i.test(fullText)) return null;

  const recordStart = fullText.search(/\bUSDOT INFORMATION\b/i);
  const text = recordStart >= 0 ? fullText.slice(recordStart) : fullText;
  ...
  const usdotStatus = getTextValue(text, 'USDOT Status', labels) || getTextValue(text, 'Operating Status', labels) || '';
  const operatingAuthorityStatus = getTextValue(text, 'Operating Authority Status', labels) || '';
  ...
  const isOutOfService = /out[-\s]+of[-\s]+service/i.test(`${usdotStatus} ${operatingAuthorityStatus}`) || Boolean(outOfServiceDate);
  const isNotAuthorized = /not\s+authorized/i.test(operatingAuthorityStatus);
  const isAuthorized = /authorized/i.test(operatingAuthorityStatus) && !isNotAuthorized;
  const statusCode = isOutOfService ? 'O' : /inactive/i.test(usdotStatus) ? 'I' : /active/i.test(usdotStatus) ? 'A' : undefined;
  const allowedToOperate = isOutOfService || isNotAuthorized ? 'N' : isAuthorized ? 'Y' : undefined;
  ...
  return {
    ...
    content: {
      carrier: {
        ...
        mcmisStatus: usdotStatus || normalizeMcmisStatus(statusCode, allowedToOperate, isOutOfService ? 'Y' : 'N'),
...
async function saferFetchSnapshot(queryParam, queryString, dotHint) {
  const timeout = withTimeout();
  const url = new URL(SAFER_QUERY_URL);
...
function mergeCarrierIdentity(primary, saferSnapshot) {
  if (!primary) return saferSnapshot;
  if (!saferSnapshot) return primary;

  const primaryContent = firstContent(primary);
  const saferContent = firstContent(saferSnapshot);
  const primaryCarrier = primaryContent?.carrier || primaryContent;
  const saferCarrier = saferContent?.carrier || saferContent;

  if (!primaryCarrier || !saferCarrier) return primary;

  return {
    ...primary,
    source: `${primary.source || 'QCMobile'}+SAFER`,
    content: {
      ...primaryContent,
      carrier: {
        ...primaryCarrier,
        // SAFER is the public FMCSA Carrier Snapshot users compare against.
        // Prefer its operating-status fields over QCMobile's record-status fields
        // so active/authorized/not-authorized carriers match the FMCSA website.
        statusCode: saferCarrier.statusCode ?? primaryCarrier.statusCode,
        allowedToOperate: saferCarrier.allowedToOperate ?? saferCarrier.allowToOperate ?? primaryCarrier.allowedToOperate,
        allowToOperate: saferCarrier.allowToOperate ?? saferCarrier.allowedToOperate ?? primaryCarrier.allowToOperate,
        outOfService: saferCarrier.outOfService ?? primaryCarrier.outOfService,
        oosDate: saferCarrier.oosDate ?? primaryCarrier.oosDate,
        outOfServiceDate: saferCarrier.outOfServiceDate ?? primaryCarrier.outOfServiceDate,
        mcmisStatus: saferCarrier.mcmisStatus ?? primaryCarrier.mcmisStatus,
        operatingAuthorityStatus: saferCarrier.operatingAuthorityStatus ?? primaryCarrier.operatingAuthorityStatus,
        mcmisCarrierStatus: saferCarrier.mcmisStatus ?? primaryCarrier.mcmisCarrierStatus,
      },
    },
  };
}
...
async function lookupCarrierIdentity(dot) {
  const saferPromise = saferFetchByDOT(dot);

  try {
    const [qcmobileIdentity, saferSnapshot] = await Promise.all([
      fmcsaFetch(`/carriers/${dot}`),
      saferPromise,
    ]);

    return mergeCarrierIdentity(qcmobileIdentity, saferSnapshot);
  } catch (error) {
...
function normalizeCarrier(raw) {
  ...
  return {
    ...
    outOfServiceDate,
    mcmisStatus,
    operatingAuthorityStatus: c.operatingAuthorityStatus,
...
function deriveAuthorityFromCarrier(carrier) {
  const status = String(carrier?.operatingAuthorityStatus || '').toUpperCase();
  if (!status) return null;

  return {
    authorizedForProperty: /PROPERTY/.test(status) ? 'Y' : undefined,
    authorizedForHHG: /HHG|HOUSEHOLD/.test(status) ? 'Y' : undefined,
    authorizedForPassenger: /PASSENGER/.test(status) ? 'Y' : undefined,
    operatingAuthorityStatus: carrier.operatingAuthorityStatus,
  };
}
...
async function buildLookupResult(dot, identity) {
  ...
  const carrier = normalizeCarrier(identity);
  ...
  return {
    ...
    authority: normalizeAuthority(authority) || deriveAuthorityFromCarrier(carrier),
